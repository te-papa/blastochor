...
from src.util.OutputRow import OutputRow
from src.util.DeletionChecker import check_for_deletions
...
class Output():
	...
	def write_output(self):
		...
		if self.label == "core":
			deletion_check = read_config("deletion_check")
			if deletion_check:
				additional_records = check_for_deletions(platform_id=deletion_check)
				if additional_records:
					for record in additional_records:
						record_row = OutputRow()
						record_row.values = record
						self.rows.append(record_row)
