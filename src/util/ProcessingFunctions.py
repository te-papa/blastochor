	...
	def __init__(self, function=None, params=None, output_row=None, input_value=None, reprocess=False, value_index=None):
		...
		if not read_config("quiet"):
			if self.output_value:
				print(f"Output value: {self.output_value}")
			else:
				print("No value for this function")

	def select_function(self):
		match self.function:
			case "continue_if":
				condition_predicate = self.params["predicate"]
				condition_value = self.params["value"]
				condition_functions = self.params["functions"]
				condition_result = None
				condition_reprocess = False
				for condition_function in condition_functions:
					condition_container = FunctionContainer(self.output_row,
					                                        condition_result,
					                                        condition_reprocess,
					                                        condition_function,
					                                        None)
					condition_result = condition_container.current_value
					condition_reprocess = condition_container.reprocess

				# Check the resulting value against what the condition requires
				...
				match condition_predicate:
					case "equals":
						if condition_value == condition_result:
					...
					case "in":
						if condition_result:
							if condition_value in condition_result:
								continue_processing = True
					case "not_in":
						if condition_result:
							if condition_value not in condition_result:
								continue_processing = True
						else:
							continue_processing = True
			...
			case "compare_numbers":
				path_one = self.params["path_one"]
				path_two = self.params["path_two"]
				use = self.params.get("use")
				self.output_value = compare_numbers(data=self.input_value,
				                                    path_one=path_one,
				                                    path_two=path_two,
				                                    use=use)
			...
			case "contains":
				requested_values = self.params["match"].split("|")
				self.output_value = contains(data=self.input_value,
											 requested_values=requested_values)

			case "count_list":
				self.output_value = count_list_items(data=self.input_value)
			...
			case "split_on":
				...
				self.output_value = split_on(data=self.input_value,
											 delimiter=delimiter)
			...
			case "wrap_value":
				self.output_value = wrap_value(data=self.input_value)
...
def compare_numbers(data=None, path_one=None, path_two=None, use="lower"):
	# Get two values and return the higher or lower. If no second value return the first
	# Use for depth or elevation comparisons
	value_one = literal(data, path=path_one)
	value_two = literal(data, path=path_two)
	if value_one and value_two:
		try:
			value_one = int(value_one)
			value_two = int(value_two)
			if use == "lower":
				return value_one if value_one < value_two else value_two
			elif use == "higher":
				return value_one if value_one > value_two else value_two
		except TypeError:
			return None
...
def contains(data, requested_values):
	# Returns True if the parameter value is in the provided data
	# Data must be a string or list of strings. If requested value is a list of terms,
	# a single match will return True
	if data:
		for requested_value in requested_values:
			requested_value = requested_value.lower()
			if isinstance(data, str):
				if requested_value in data.lower():
					return True
			elif isinstance(data, list):
				for value in data:
					if isinstance(value, str):
						if requested_value in value.lower():
							return True
...
def count_list_items(data):
	# Count the length of a provided list value
	if data:
		return len(data)
...
def process_quality_score(data):
	...
	if stats.quality_score_lower and stats.quality_score_upper:
		try:
			...
			modifier = random.randint(-30, 30)
			priority = priority + modifier

			if priority <= 0:
				priority = random.randint(1, 90)
...
def split_on(data, delimiter):
	# Split a string value into a list. Will be re-combined using "|" at write
	if data:
		if isinstance(data, str):
			value_list = data.split(delimiter)
			return value_list
		return data
